 ...
 */
export class DartRuntimeGenerator {
  private readonly code: CodeMaker;
  private readonly typeresolver: DartTypeResolver;
  private readonly nameutils: DartNameUtils = new DartNameUtils();

  public constructor(code: CodeMaker, typeresolver: DartTypeResolver) {
    this.code = code;
  ...
  public createInvokeMethodIdentifier(
    ...
    if (method.parameters) {
      for (const param of method.parameters) {
        paramTypes.push(
          `typeof(${this.typeresolver.toDartType(param.type)}${
            param.variadic ? '[]' : ''
          })`,
        );
