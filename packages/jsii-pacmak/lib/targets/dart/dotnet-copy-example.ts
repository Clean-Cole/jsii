...
import { DartRuntimeGenerator } from './dart-runtime-generator';
import { DartTypeResolver } from './dart-type-resolver';
import { DartFileGenerator } from './file-generator';
...
class DartGenerator extends Generator {
  ...
  private dartRuntimeGenerator!: DartRuntimeGenerator;
  ...
  public async save(
    ...
    const filegen = new DartFileGenerator(
      this.assembly,
      tarballFileName,
      this.code,
    );
    ...
    await fs.copyFile(tarball, path.join(outdir, pubName, tarballFileName));

    // Create an anchor file for the current model
    this.generateDependencyAnchorFile();

    if (license) {
      await fs.writeFile(path.join(outdir, pubName, 'LICENSE'), license, {
        encoding: 'utf8',
      });
    }
    if (notice) {
      await fs.writeFile(path.join(outdir, pubName, 'NOTICE'), notice, {
        encoding: 'utf8',
      });
    }
  ...
  protected generateDependencyAnchorFile() {
    const namespace = `${
      this.assembly.targets!.dart!.pubName
  ...
  protected onBeginNamespace(jsiiNs: string) {
    if (jsiiNs == null) console.log(jsiiNs);
  ...
  protected onInterfaceMethod(ifc: spec.InterfaceType, method: spec.Method) {
    if (ifc == null) console.log(ifc);
    // this.dartDocGenerator.emitDocs(method, {
    //   api: 'member',
    //   fqn: ifc.fqn,
    //   memberName: method.name,
    // });
    // this.dartRuntimeGenerator.emitAttributesForMethod(ifc, method);
  ...
  protected onInterfaceProperty(ifc: spec.InterfaceType, prop: spec.Property) {
    ...
    // this.dartRuntimeGenerator.emitAttributesForProperty(prop);

    const propType = this.typeresolver.toDartType(prop.type);
    const propName = this.nameutils.convertPropertyName(prop.name);

    if (prop.optional) {
      // TODO: how will we treat optional properties?
  ...
  protected onBeginClass(cls: spec.ClassType, abstract: boolean) {
    ...
    if (!cls.base) {
      ...
      const classBase = this.typeresolver.toDartType({ fqn: cls.base });
    ...
    const implementsExpr = '';
    // this.dartDocGenerator.emitDocs(cls, {
    //   api: 'type',
    //   fqn: cls.fqn,
    // });

    // TODO: Dart decorator for class attributes
    // this.dartRuntimeGenerator.emitAttributesForClass(cls);
    ...
    if (initializer) {
      // this.dartDocGenerator.emitDocs(initializer, {
      //   api: 'initializer',
      //   fqn: cls.fqn,
      // });
      this.dartRuntimeGenerator.emitDeprecatedAttributeIfNecessary(initializer);
      ...
      this.code.openBlock(
        `${className}(${parametersDefinition}): base(new DeputyProps(${args}))`,
      );
      this.code.closeBlock();
      this.code.line();
    }
    ...
    this.code.openBlock(
      `${className}(ByRefValue reference)implements base(reference)`,
    );
    this.code.closeBlock();
    this.code.line();
  ...
  protected onBeginEnum(enm: spec.EnumType) {
    ...
    this.emitNewLineIfNecessary();
    this.dartDocGenerator.emitDocs(enm, {
      api: 'type',
      fqn: enm.fqn,
    });
    // this.dartRuntimeGenerator.emitAttributesForEnum(enm, enumName);
  ...
  protected onEnumMember(enm: spec.EnumType, member: spec.EnumMember) {
    this.dartDocGenerator.emitDocs(member, {
      api: 'member',
      fqn: enm.fqn,
      memberName: member.name,
    });
    const enumMemberName = this.nameutils.convertEnumMemberName(member.name);
    // this.dartRuntimeGenerator.emitAttributesForEnumMember(
    //   enumMemberName,
    //   member,
    // );
  ...
  private namespaceFor(assm: spec.Assembly, type: spec.Type): string {
    ...
    return assm.targets!.dart!.pubName;
  }
  ...
  private emitMethod(
    ...
    if (definingType == null) console.log(definingType);
    this.emitNewLineIfNecessary();
    const returnType = method.returns
      ? this.typeresolver.toDartType(method.returns.type)
      : 'void';
    ...
    //   memberName: method.name,
    // });
    // this.dartRuntimeGenerator.emitAttributesForMethod(
    //   cls,
    ...
    if (method.abstract) {
      ...
      this.code.line(
        this.dartRuntimeGenerator.createInvokeMethodIdentifier(
          method,
  ...
  private renderParametersString(
    ...
    if (parameters) {
      for (const p of parameters) {
        ...
        let type = this.typeresolver.toDartType(p.type);
        if (p.optional) {
  ...
  private emitInterfaceProxy(ifc: spec.InterfaceType | spec.ClassType): void {
    ...
    this.code.line();
    this.dartDocGenerator.emitDocs(ifc, {
      api: 'type',
      fqn: ifc.fqn,
    });
    this.dartRuntimeGenerator.emitAttributesForInterfaceProxy(ifc);

    const interfaceFqn = this.typeresolver.toNativeFqn(ifc.fqn);
    // const suffix = `implements ${interfaceFqn}`;
    // const newModifier = this.proxyMustUseNewModifier(ifc) ? 'new ' : '';

    this.code.openBlock(`interface ${interfaceFqn}`);
  ...
  private emitInterfaceDataType(ifc: spec.InterfaceType): void {
    ...
    // });
    const suffix = '';
    // const suffix = `implements ${this.typeresolver.toNativeFqn(ifc.fqn)}`;
    // this.dartRuntimeGenerator.emitAttributesForInterfaceDatatype(ifc);
    this.code.openBlock(`class ${name} ${suffix}`);
    this.flagFirstMemberWritten(false);
    const datatype = true;
    const proxy = false;
    this.emitInterfaceMembersForProxyOrDatatype(ifc, datatype, proxy);
      ...
      for (const method of currentType.methods ?? []) {
        ...
        if (method.parameters) {
          method.parameters.forEach((param) => {
            methodParameters += `;${this.typeresolver.toDartType(param.type)}`;
          });
        }
  ...
  private emitProperty(
    ...
    // TODO: Likely we want to bring this back as a decorator for the runtime
    // this.dartRuntimeGenerator.emitAttributesForProperty(prop);
    ...
    const propTypeFQN = this.typeresolver.toDartType(prop.type);
    const isOptional = prop.optional ? '?' : '';
  ...
  private emitConstProperty(cls: spec.ClassType, prop: spec.Property): void {
    ...
    // TODO: big change out for emitting property attributes here
    // this.dartRuntimeGenerator.emitAttributesForProperty(prop);
    // const access = this.renderAccessLevel(prop);
    const propName = this.nameutils.convertPropertyName(prop.name);
    const staticKeyword = prop.static ? 'static ' : '';

    this.code.line(`${staticKeyword}${propType}${isOptional} ${propName}`);
    // this.code.line('get;');
    this.code.closeBlock();
    const className = this.typeresolver.toNativeFqn(cls.fqn);
  ...
  private renderAccessLevel(method: spec.Method | spec.Property): string {
    return method.protected ? 'private' : '';
  }
  ...
  private toDartFilePath(type: string): string {
    return `${type}.dart`;
  }
  ...
  private openFileIfNeeded(
    ...
    const pubName = this.assembly.targets?.dart?.pubName;
    if (!pubName) {
      throw new Error(
        `The module ${this.assembly.name} does not have a dotnet.packageId setting`,
      );
    }
    const filePath = toSnakeCase(namespace.replace(/[.]/g, '_'));
    this.code.openFile(
      path.join(pubName, filePath, this.toDartFilePath(typeName)),
    );
    if (usingDeputy) {
      // TODO: Replace with imported Dart runtime
  ...
  private closeFileIfNeeded(
    ...
    if (!pubName) {
      throw new Error(
        `The module ${this.assembly.name} does not have a pubName specified in the package.json`,
      );
    }
    const filePath = toSnakeCase(namespace.replace(/[.]/g, '_'));
    this.code.closeFile(
  ...
  private emitNamespaceDocs(
    ...
    this.dartDocGenerator.emitMarkdownAsRemarks(docSource.readme.markdown, {
      api: 'moduleReadme',
      moduleFqn: jsiiFqn,
    });
    this.emitHideAttribute();
