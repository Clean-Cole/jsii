...
export class DartTypeResolver {
  // The dependency tree for the current jsii input model.
  // This is later used to output the csproj
  public namespaceDependencies: Map<string, DartDependency> = new Map<
    string,
    DartDependency
  >();
  ...
  /**
   * Translates any jsii type to its corresponding Dart type
   */
  ...
  public resolveNamespace(
    ...
    let resolved = assm.targets?.dart?.pubName;
    if (!resolved) {
      throw new Error(
   ...
   */
  private toDartCollection(ref: spec.CollectionTypeReference): string {
    const elementDartType = this.toDartType(ref.collection.elementtype);
    switch (ref.collection.kind) {
      case spec.CollectionKind.Array:
        return `List<${elementDartType}>`;
      case spec.CollectionKind.Map:
        return `Map<String, ${elementDartType}>`;
      default:
        throw new Error(
